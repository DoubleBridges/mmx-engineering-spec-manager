...
def test_sink_callout_model_creation(db_session):
    """
    Test that the SinkCallout model can be created and linked to a Project and SpecificationGroup.
    """
    project = Project(
        number="101",
        name="Test Project",
    ...
    sink_callout = SinkCallout(
        material="SINK",
        tag="SK1",
        description="TBD, ARCH/GC PLEASE SPECIFY, 14 24",
        project_id=project.id,
        specification_group_id=spec_group.id
    )
    db_session.add(sink_callout)
    db_session.commit()

    retrieved_callout = db_session.query(SinkCallout).filter_by(tag="SK1").first()

    assert retrieved_callout.material == "SINK"
    assert retrieved_callout.tag == "SK1"
    assert retrieved_callout.description == "TBD, ARCH/GC PLEASE SPECIFY, 14 24"
